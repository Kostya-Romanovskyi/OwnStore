...
const MainInfoMovie = () => {
    ...
    const handleDeleteMovie = () => {
        librarySel.data &&
            librarySel.data.map((oneMovie) => {
                if (movieId === oneMovie.movieId) {
                    dispatch(
                        deleteFirebaseData({ uid: authInfoSel.uid, id: oneMovie.id })
                    );
    ...
    return (
        <>
            <TitleWrapp>
                <BackButton
                    to={
                        backPath?.pathname + backPath?.search
                            ? backPath?.pathname + backPath?.search
                            : "/"
                    }>
                    Go back
                ...
                {imagesSel && imagesSel.length !== 0 ? (
                    <GallerysSwiper images={imagesSel} />
                ) : (
                    <Plug>We haven`t images off this movies</Plug>
                )}

                <MainButton
                    route={`/search/${movieId}/cast`}
                    content={"Cast"}
                    state={{ from: location }}
                />
